# Lista de endpoints (Backend - FastAPI) — SideQuest
...
- Autenticación
  - POST /api/v1/auth/register — Registro de usuario (nombre, email, contraseña).
  - POST /api/v1/auth/login — Login; devuelve token de acceso (JWT) y opcional refresh token.
...
- Planes
  - POST /api/v1/plans — Crear un plan (título, descripción, fechas, visibilidad, tags).
  - GET /api/v1/plans — Listar planes (paginación + filtros: owner, tags, fechas, compartidos).
  ...
  - DELETE /api/v1/plans/{id} — Eliminar plan (soft-delete opcional).
...
- Otros
  - Documentación automática: OpenAPI/Swagger en `/docs` (FastAPI) y health checks para integraciones.
